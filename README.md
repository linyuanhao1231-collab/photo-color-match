...
還有第二條完全不同的路：不需要 before/after 配對，直接套一個手刻的風格。
```bash
uv run colormatch.py look --style portra400 --preview a.jpg b.jpg --cube-out portra400.cube
uv run colormatch.py look --style wkw --input ./raw --out ./done --grain 0.02
```
可用風格：`portra400`（Kodak Portra 400 底片模擬）、`wkw`（王家衛式 teal-orange 電影感）。
見下方「手刻風格 LUT」一節。
...
跟前面「從 before/after 反解」是完全不同的機制：這裡不用任何照片當訓練資料，
是直接照公開可查的美學特徵手刻參數（對比曲線、高光肩部、陰影/高光分離染色、
選擇性的膚色加暖 / 綠色降飽和 / 紅色加飽和），純函數合成一個風格。

跟「復刻某張特定照片的色調」（見上面的討論）不是同一件事——那個需要語意層級比對
單一參考圖，做不到；這裡做的是「已知的一類風格公式」，本來就不依賴任何參考照片。

- `portra400`：Kodak Portra 400 人像底片的特色——寬容度高、反差低、皮膚亮部
  不死白、整體偏暖、綠/藍自然收斂不搶戲。
- `wkw`：王家衛 / 杜可風式的 teal-orange 電影感——陰影推青、高光推暖橘、
  黑位微微抬起的霧面感、紅色（旗袍/霓虹）特別飽和突出。

**這是手刻的近似，不是校準過真實底片掃描片或電影機的精密模擬**——沒有拿實際
Portra 400 沖洗掃描或《花樣年華》的畫面比對過參數，只是照普遍認知的特徵寫的一個
起點。回家用真實照片跑 `--preview` 看效果、依你的審美調整 `RECIPES` 裡的參數
（`colormatch.py` 裡的 `RECIPES` 字典），這一步沒辦法用合成測試圖驗證到位——
需要真正有動態範圍、有皮膚、有天空的照片才看得出高光滾降、選擇性染色到底對不對味。

底片顆粒（`--grain`）是空間雜訊紋理，不是顏色轉換，進不了 `.cube`，只在
`--preview` 或批次輸出時套用；用 `.cube` 當 Lightroom Profile 的話，顆粒感
要靠 Lightroom 自己的 Grain 面板另外加。
